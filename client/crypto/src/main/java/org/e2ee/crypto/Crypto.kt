...
fun initAccount(): Pair<X3DHKeyManager,PreKeyBundle>{
...
class Crypto{
    ...
    fun decryptPreKeyMessage(
        ...
        val (hkr, nhks) = kdf.initHeaderKeyKDF(
            ...
            sKReceiver,
            ecdh.performDH(
                decryptionDto.receiverKeyManager.signedPreKeyPair.private,
                util.decodePublicKey(decryptionDto.message.EKs))
        )

        var receiverState = doubleRatchet.ratchetInitReceiverHE(
            sKReceiver,
            decryptionDto.receiverKeyManager.signedPreKeyPair,
            hkr,
            nhks
        )
    ...
    fun generateSPK(
        ...
        val signature = sig.signMessage(
            ...
            keyManager.identitySigningKey.private
        )

        return Pair(signedPreKey, signature)
    }
    ...
    fun generateOPK(
        ...
        return keyManager.generateOPK(count, keyManager.opkStore)
    }
}