...
interface KeyManagerApi {
    ...
    suspend fun updateSignedPreKey(
        @Body request: UpdateSignedPreKeyBundle
    ): Response<Unit>

    @POST("/api/v1/keymanager/updateOPK")
    suspend fun updateOneTimePreKeys(
        @Body request: UpdateOpkKeys
    ): Response<Unit>

    @POST("/api/v1/keymanager/updatePreKeyBundle")
    suspend fun updatePreKeyBundle(
        @Body request: PreKeyBundleDto
    ): Response<Unit>
}